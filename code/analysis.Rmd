...
up = two_sig_rxn[two_sig_rxn$coefficient > 0, ]
down = two_sig_rxn[two_sig_rxn$coefficient < 0, ]
...
pvals = data.frame(name = as.character(), pval = as.numeric())
...
for(t in all_terms) {
 ...
 # print(t)
  ...
  pvals[nrow(pvals) + 1, ] = c(t, pp_value2$p.value)
}
pvals$p.adj = p.adjust(pvals$pval, method = "BH")
pvals = pvals[pvals$p.adj < 0.05, ]
return(pvals)
}
terms_up = calc_terms(up)
terms_down = calc_terms(down)
both = intersect(terms_down$name, terms_up$name)
```

```{r}
map = data.frame(rxn = as.character(), subs = as.character())

for(t in terms_up$name)
{
  ...
  for(r in names(enrichment_map))
  {
    ...
    {
      # DO I WANT TO INCLUDE ALL ACTIVE REACTIONS OR JUST THE SIGNIFICANT ASSOC ONES
      if( r %in% lst_allRxns)
      {
...
g <- graph_from_adjacency_matrix(overlap, mode = "undirected", weighted = TRUE)
...
enricher(gene = unique(df_corInternRxnFluxToAge[df_corInternRxnFluxToAge$p.adj <= 0.05 &
                                                                          df_corInternRxnFluxToAge$Spearman_rho > 0,"RxnFlux"]),
                      universe = rownames(df_internalReactionFluxes))# calculate richness and Chao1 using vegan package
